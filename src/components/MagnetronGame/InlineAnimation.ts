...
export type InlineAnimationType = {
    duration?: number
    looping?: boolean
    start?: (game: Magnetron, currDuration: number, duration: number) => void
    update?: (game: Magnetron, deltaTime: number, currDuration: number, duration: number) => void
    end?: (game: Magnetron, currDuration: number, duration: number) => void
}
...
export class InlineAnimation extends Animation {
    ...
    protected start(game: Magnetron): void {
        if (this.inlineAnimation.start) {
            this.inlineAnimation.start(game, this.currDuration, this.duration)
        }
    }

    protected update(game: Magnetron, deltaTime: number): void {
        if (this.inlineAnimation.update) {
            this.inlineAnimation.update(game, deltaTime, this.currDuration, this.duration)
        }
    }

    protected end(game: Magnetron): void {
        if (this.inlineAnimation.end) {
            this.inlineAnimation.end(game, this.currDuration, this.duration)
        }
    }
}
